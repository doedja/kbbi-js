...
class KBBIParser {
  ...
  extractExamples($li) {
    ...
    if ($exampleElements.length) {
      $exampleElements.each((_, el) => {
        const text = this.$(el).text().trim();
        // KBBI wraps each real example in a grey <font>. Extra grey spans hold
        // separators like `;` or whitespace; drop those so they do not surface
        // as `{ teks: ";" }` entries and offset the real example numbers.
        if (!text || /^[\s;:.,]+$/.test(text)) return;
        examples.push({
          nomor: examples.length + 1,
          teks: text
        });
      });
    } else {
  ...
  extractEtymology($h2) {
    ...
    if (match && match[1]) {
      ...
      // Skip the boilerplate KBBI shows to anonymous users in place of the real
      // etymology block. Returning it would just leak the gate text into JSON
      // output where a `null` is the honest answer.
      if (/Informasi etimologi hanya tersedia bagi pengguna terdaftar/i.test(cleanText)) {
        return null;
      }
