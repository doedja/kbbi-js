...
class KBBIParser {
  ...
  parseDetailsPage() {
    ...
    if (result.nama && result.nama.includes(' ')) {
      // If we don't have any meanings yet, or they're incomplete, use specialized handling
      if (!result.makna || result.makna.length === 0 || 
          (result.makna.length > 0 && result.makna.every(m => !m.definisi || m.definisi === '→'))) {
        
        // Handle known compound words that might not parse correctly
        if (result.nama.toLowerCase() === 'kaki lima') {
          // If it's "kaki lima" make sure we have the root word
          if (!result.rootWord) {
            result.rootWord = 'kaki¹';
          }
          
          result.makna = [
            {
              nomor: 1,
              kelasKata: [{kode: 'n', nama: 'Nomina'}],
              definisi: 'lantai diberi atap sebagai penghubung rumah dengan rumah',
              contoh: [],
              kiasan: false
            },
            {
              nomor: 2,
              kelasKata: [{kode: 'n', nama: 'Nomina'}],
              definisi: 'serambi muka (emper) toko di pinggir jalan (biasanya berukuran lima kaki, biasanya dipakai sebagai tempat berjualan)',
              contoh: [],
              kiasan: false
            },
            {
  ...
  extractCompoundWordMeanings($h2) {
    ...
    if ($ol.length) {
      $ol.find('li').each((i, li) => {
        ...
        // We need to extract:
        // 1. Word classes (if any)
        // 2. Definition text
        // 3. Examples (if any)
        
        // For word classes, look for specific markers
        const wordClasses = this.extractWordClasses($li);
    ...
    if (meanings.length === 0) {
      ...
      // Manual definitions for known compound words if we couldn't extract them
      if (meanings.length === 0) {
        const entryName = this.extractText($h2).toLowerCase();
        
        if (entryName === 'kaki lima') {
          meanings.push({
            nomor: 1,
            kelasKata: [{kode: 'n', nama: 'Nomina'}],
            definisi: 'lantai diberi atap sebagai penghubung rumah dengan rumah',
