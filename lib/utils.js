...
const KBBI_HOST = 'kbbi.kemendikdasmen.go.id';
const KBBI_ORIGIN = `https://${KBBI_HOST}`;

class Utils {
  static get HOST() { return KBBI_HOST; }
  static get ORIGIN() { return KBBI_ORIGIN; }
  static get LOGIN_URL() { return `${KBBI_ORIGIN}/Account/Login`; }
  static buildDetailsUrl(eid) { return `${KBBI_ORIGIN}/DataDasarEntri/Details?eid=${encodeURIComponent(eid)}`; }
  ...
  static buildUrl(word) {
    return `${KBBI_ORIGIN}/entri/${encodeURIComponent(word)}`;
  }
